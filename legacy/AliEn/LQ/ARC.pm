...
# 
# maintainers: Maarten.Litmaath@cern.ch, Miguel.Martinez.Pedreira@cern.ch
# 
...
sub submit {
	...
	my @command = ("$fix_env arcsub -f $xrslfile "
...
sub kill {
	...
	# my $error = $self->_system( "$fix_env arckill", "$contact" );
	my (@output, $error) = $self->_system( "$fix_env arckill $contact" );
	return $error;
}
...
sub getOutputFile {
	...
	$self->_system(
	    "$fix_env arccp $batchId/alien-job-$queueId/$output /tmp/alien-output-$queueId");
...
sub cleanAllFinished {
	...
	$self->_system("$fix_env arcclean -a -s FINISHED");
...
#    }
	...
	open LB, "$fix_env arcstat -a 2>&1 |" or next;
	my @output = <LB>;
	close LB;
...
# Remove completed jobs, unless we are in debug mode
				...
				and not $self->{LOGGER}->getDebugLevel() )
			{
				...
				if ( @completedJobs >= 500 ) {
					...
					$self->_system( "$fix_env arcclean " . join( " ", @completedJobs ) );
					@completedJobs = ();
				}
			}
...
/^(ACCEPTING)|(ACCEPTED)|(PREPARING)|(PREPARED)|(SUBMITTING)|(INLRMS:Q)$/
	...
	if ($needArcSync) {
		...
		foreach( @hosts_to_sync ){
			$self->_system( "$fix_env arcsync -T -f -c " . $_ );
...
sub getNumberRunning() {
	...
	return $r[0] + $r[1];   # the caller expects running + queued...
}
...
sub repairJobsFile{
	...
	# run system command
	my $start_time = time();
	$self->_system( "$fix_env arcsync -T -f $ce_list" );
