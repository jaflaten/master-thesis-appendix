...
sub checkConfigFile {
  ...
  foreach my $service (@SERVICES) {
    ...
    $mode =~ /add/ and
      $self->info("The local configuration is not allowed to define services")
      and next;
    foreach my $d (@blocks) {
      my $name = ${$d}[1];
      $name
        or print
        "Warning! the service $service doesn't have any names (ignoring it)\n"
        and next;
      $DEBUG
        and
        $self->debug( 2, "Defining the service $service ${$d}[0] and ${$d}[1]"
        );
      $self->{$service} = $name;
...
sub getVOLDAP {
  my $self = shift;
  my ( $host, $dn );
  # Example in .alien/Environment: export ALIEN_LDAP_DN=alice-ldap.cern.ch:8389/o=alice,dc=cern,dc=ch
...
sub GetOrganisation {
  ...
  if ( -f "$ENV{ALIEN_ROOT}/share/alien/ALIEN_VERSION" ) {
    ...
    if ( join( "", @common ) =~ /AliEn\s+(\S+),\s+build:\s*(\S+),/ ) {
      ...
      $self->info(
"Error getting the version from $ENV{ALIEN_ROOT}/share/alien/ALIEN_VERSION (wrong format?)"
      );
      return;
    }
  ...
  while (1) {
    eval {
      $config =
        ...
        ->proxy("http://$cluster:$port")->GetConfiguration();
    };
    if ($@) {
      $self->info("It died: $@");
      return;
...
sub CheckServiceCache {
  ...
  $self->{CACHE}->{$service}->{$name}->{value} =
    $self->CheckService( $service, $name, @_ )
    or return;
...
sub GetgContainer {
  ...
  for ( my $i = 0 ; $i < $total ; ++$i ) {
    push @{ $self->{G_CONTAINER}->{JUDGES_LIST} },
      $self->getAttributes( $mesg->entry($i), 1 );
  }
  ...
  for ( my $i = 0 ; $i < $total ; ++$i ) {
    ...
    if ( $judgeMesg->count ) {
      ...
      for ( my $i = 0 ; $i < $count ; ++$i ) {
        push @{ $service->{JUDGES_LIST} },
          $self->getAttributes( $judgeMesg->entry($i), 1 );
      }
    }
