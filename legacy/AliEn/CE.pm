...
sub submitCommand {
	...
	$DEBUG and $self->debug(1, "zoption $zoption - noption $noption - Arg: ".Dumper(@arg));

	my $user = $self->{CATALOG}->{CATALOG}->{ROLE};

	my @quotas = $self->{CATALOG}->{CATALOG}->checkFileQuota($user, 0);
	if (@quotas) {
		# prevent job submission @ 95% filled quotas
		if (($quotas[0]!=1) || ($quotas[2] and $quotas[2] >= 0.95) || ($quotas[3] and $quotas[3] >= 0.95)) {

		  my $relativeSizeUsage = sprintf("%.2f%%", $quotas[2] * 100);
		  my $relativeFileUsage = sprintf("%.2f%%", $quotas[3] * 100); 


		  $self->info("ERROR!!!! Your file quota is >95% full!!! -> Aborting job submission!");
			$self->{LOGGER}->error("CE", "=====================================================");
			$self->{LOGGER}->error("CE", " File quotas full. Aborting job submission");
			$self->{LOGGER}->error("CE", "=====================================================");

		  my $userInfo = AliEn::Util::getUserInfoFromLDAP($user, $self->{CONFIG});
		  
			my $userCN = $userInfo->get_value('cn');
			my $mail = AliEn::Util::sendMail({
				to => $userInfo->get_value('email'),
				cc => 'alice-grid-admins@cern.ch',
				from => 'alienmaster@cern.ch',
				subject => "AliEn: Account $user urgently needs your attention",
				content => "Dear $userCN,

The job submission by account $user has been denied due to
exhaustion of your quotas: file (utilization: $relativeFileUsage) or disk space (utilization: $relativeSizeUsage).

Please clean up your home directory and keep it below 95% of the quotas.
You can consult the limits and usage at
https://alimonitor.cern.ch/users/quotas.jsp
...
$after";
  ...
  if($ENV{CE_SINGULARITY}){
  	...
  	$content =~ s/\n$//g;
  	$content =~ s/'/'\\''/g;  	
  	# singularity exec --bind /cvmfs:/cvmfs --contain /cvmfs/atlas.cern.ch/repo/images/singularity/x86_64-centos7.img bash -c 'printf '\''ALIEN_DEBUG=-d:Trace alien RunAgent\n'\'' > agent.startup.$$.sh && chmod +x agent.startup.11838.sh && ./agent.startup.$$.sh'
    $content = $ENV{CE_SINGULARITY} . " bash -c 'printf \"$content\" > agent.startup.$$.sh && chmod +x agent.startup.$$.sh && ./agent.startup.$$.sh'";
  }
  ...
  if (!$returnContent) {
	...
	!$ENV{CE_SINGULARITY} and print FILE "#!/bin/bash\n";
	print FILE "$content";
	close FILE;
	chmod 0750, $script;
	$content = $script;
...
sub resyncJobAgent {
	...
	my $jobs = 
	$self->{TASK_DB}->query(
		"select q.userid, q.queueid, q.agentid, qj.origjdl jdl from QUEUE q left join JOBAGENT j on entryid=agentid join QUEUEJDL qj on q.queueId=qj.queueId 
		WHERE statusid=5 AND entryid is null") 
		or $self->info("Error getting the jobs without jobagents")
		and return;

	foreach my $job (@$jobs) {
		$self->info(Dumper($job));
		$self->info("We have to fix the jobagent entry for $job->{queueid}"); # (agentId: ".(defined($job->{agentid}) ? "$job->{agentid}" : "null").")

		my $job_ca = Classad::Classad->new($job->{jdl});
		if (!$job_ca->isOK()) {
			$self->info("Failed creating classad for job $job->{queueid}");
			return;
		}
  		
		my ($ok, $req) = $job_ca->evaluateExpression("Requirements");
		my $agentReq = AliEn::Service::Optimizer::Job::getJobAgentRequirements($self, $req, $job_ca);
		my $agentId = $self->{TASK_DB}->insertJobAgent($agentReq);
		$self->{TASK_DB}->do("UPDATE QUEUE set agentId=? where queueId=?", {bind_values => [$agentId,$job->{queueid}]});
	}
...
Usage:
  ...
  jquota set <user> <field> <value> - set the user quota for job
                                      (maxUnfinishedJobs, maxTotalCpuCost, maxTotalRunningTime, maxParallelJobs)
