...
sub diff {
  ...
  # mute the alien output and get the files from their lfns
  # the output needs to be muted because people might call diff -c to create patch files
  # and the extra output would corrupt the patchfile
  $self->partSilent();
  my $file1 = $self->get($lfn1);
  my $file2 = $self->get($lfn2);
  $self->restoreSilent();

  # check if we were able getting the files
  ($file1 and $file2) or  
    $self->info( "Error getting the files for diff! ") 
    and return;

  # join the args (if any)
  my @cmdArgs = ($file1, $file2, @args);

  system("diff", @cmdArgs) and return;
}
...
sub mirror {
  ...
  my $opt={};
  @ARGV=@_;
  Getopt::Long::GetOptions($opt,  "f", "g", "m=i", "b","t", "u", "r", "c=s","try=i", "w", "x=s") or 
  ...
  my $exse_site;
  if($opt->{x}){	
  	$opt->{x} =~ /ALICE::(\w+)::\w+/i and $exse_site = $1;
  	$self->info("Excluding mirror to go to $exse_site and $opt->{x}");
  }

  my $at = $se;
  $opt->{x} and $at = "SE to be decided by TransferManager (excluding $exse_site)";
  $self->info( "Mirroring file $realLfn at $at");
...
#  $opt->{g} and $transfer->{transferGroup}=$opt->{g};
  ...
  $exse_site and $transfer->{excludeSite}=$exse_site; 
