...
sub f_addUser {
  ...
  $self->{PRIORITY_DB} or 
    $self->{PRIORITY_DB} = AliEn::Database::TaskQueue->new({ROLE=>'admin',SKIP_CHECK_TABLES=> 1});
...
sub f_renumber {
 ...
 ( $self->{ROLE}  =~ /^admin(ssl)?$/ ) or
  ...
  for my $d (@$info) {
    $d->{lfn} =~ /^$lfn$/ or next;
    $self->info("Renumbering the table $d->{tableName} ($d->{lfn})" );
    my $error= $self->{DATABASE}->renumberLFNtable("L$d->{tableName}L", $options);
    $self->info("The renumbering returned $error");
  }
  return 0;
}
...
sub setSEStatus {
  ...
  my $set={};
        ...
        if (keys(%$set)){
                ...
                $self->{DATABASE}->{LFN_DB}->update('SE', $set, "sename=?", {bind_values=>[$sename]});
...
sub listSEDistance_HELP {
 ...
 listSEDistance [<site>] [read [<lfn>]|write] 


 Options:
   <site>: site name. Default: current site
   [read|write]: action. Default write. In the case of read, if an lfn is specified, use only SE that contain that file
  "
}
...
sub listSEDistance {
       ...
       my $readSE="";
       if ($file){
          my @info=$self->f_whereis("l", $file);
          @info or $self->info("Error getting the whereis of $file") and return;
          $readSE = " and sename in (";
          foreach my $s (@info){
             $readSE .= "'$s',";
          }
          $readSE =~ s/,$/\)/;
        ...
        my $se=$self->{DATABASE}->query("select sename, sedemotewrite, sedemoteread, distance, distance + sedemote$action weight
        from SE join SEDistance using (seNumber)  where sitename=? $readSE order by 5 asc ", undef, {bind_values=>[$sitename]});

        $se or $self->info("Error doing the query in listSEDistance") and return;

        $self->info("The ordered list $action is:");
        foreach my $e (@$se){
          $self->info("$e->{weight}     $e->{sename} (distance: $e->{distance} read: $e->{sedemoteread} write: $e->{sedemotewrite}) \n",undef, 0)
        }
...
sub refreshSEDistance {
  ...
  my $sitename = shift;
	...
	if ($sitename){
		...
		my $e=$self->{DATABASE}->{LFN_DB}->{FIRST_DB}->queryColumn("select distinct sitename from SEDistance");
		@todo=@$e;		
	}
...
#  	  $self->info("We couldn't get the info from ML. Putting all the ses");
      @selist = @{$self->{DATABASE}->{LFN_DB}->{FIRST_DB}->query("select distinct seName, if(locate(?,sename)>1, 0, 0.5) distance  from SE", undef,
    		{bind_values=>[$site]})};
  	}
  	$site and (scalar(@selist) gt 0) or $self->info("No site or selist\n") and return 0;
	$self->info("Going to lock SE(r) and SEDistance(w)");
  	$self->{DATABASE}->{LFN_DB}->{FIRST_DB}->lock("SE read, SEDistance");
  	$self->info("Locked SE(r) and SEDistance(w)");
  	eval{
  		$self->{DATABASE}->{LFN_DB}->{FIRST_DB}->do("delete from SEDistance where sitename=?", {bind_values => [$site]});
  	};
...
sub resyncLDAPSE {
  ...
  if ($total == 0){
  	$self->info("There was a problem getting the AliEnMSSs or nothing to do!");
  	$transfers->close();
  	$ldap->unbind();
  	return;	
  }
...
sub cleanupGUIDCatalogue{
  ...
  if($self->{DATABASE}->{GUID_DB}->checkPermission("w",$file->{guid})) {
    ...
    if ($file->{pfn}=~ m{^\*$}){
      ...
      my $ref= $guiddb->query("SELECT pfn, senumber
      ...
      $guiddb->do("delete from g using $table join ${table}_PFN g using (guidid ) where guid=string2binary(?) and pfn=?", {bind_values=>[$file->{guid}, $file->{pfn}]});
    }
  }
  return @pfns;
}
...
sub calculateFileQuota {
  my $self = shift;
  my $silent = shift;
  # if $userFilter set to 1 we will skip every entry whose lfn does not match with $self->{CONFIG}->{USER_DIR}
  # if set to 0 (default behaviour) we will calculate quotas for all users and if set to -1 we will only recalculate for production users
  my $userFilter = shift || 0;
  # default filter -> allow all lfns
  my $filter;
  # users-only filter -> lfns must be placed in user dir
  if($userFilter and $userFilter eq "1"){ $filter = qr/^$self->{CONFIG}->{USER_DIR}/;}
  # user-exclusive filter -> no lfns from user dir allowed
  if($userFilter and $userFilter eq "2"){ $filter = qr/^(?!$self->{CONFIG}->{USER_DIR})/;}
 ...
 ( $self->{ROLE}  =~ /^admin(ssl)?$/ ) or
  ...
  foreach my $h (@$rhosts){
    ...
    if($filter){
      $h->{lfn} =~ $filter and $self->debug(1, $h->{lfn} . ' matches ' . $filter) or $self->debug(1, $h->{lfn} . ' does not match ' . $filter) and next;
    }
...
#    $db->queryValue("select 1 from (select max(ctime) ctime, count(1) counter from $LTableName) a left join LL_ACTIONS on tablenumber=? and action='QUOTA' where extra is null or extra<>counter or time is null or time<ctime", undef, {bind_values=>[$LTableIdx]}) or next;
    ...
    my $fquotaL = $db->query("select l.owner as user, count(l.lfn) as nbFiles, sum(l.size) as totSize from ${LTableName} l 
           where l.type='f' group by l.owner order by l.owner");
    
    if ( scalar(@$fquotaL) ){
      my $quotaInsert = "insert into ${LTableName}_QUOTA (user, nbFiles, totalSize) values ";
