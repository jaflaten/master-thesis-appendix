...
sub  getBaseEnvelopeForWriteAccess {
  ...
  ($ok eq 1) or $self->info($message,1) and return 0;
...
sub prepookArchiveLinksInBookingTable{
  ...
  foreach my $link (@links){
      ...
      my $env = $self->getBaseEnvelopeForWriteAccess($user, $l, $s, $m, $g)
        or $self->info("Authorize: The requested link $l could not been booked or had permission denied")
        and return 0;      
      $env->{turl} = "guid:///$archGUID?ZIP=".(defined $p ? $p : "").$self->f_basename($env->{lfn});
      $env->{se} = "no_se";
...
sub  getSEsForWriteOrMirrorAccess{
  my $self=shift;
  my $user=(shift || return 0);
...
sub authorize{
  ...
  if ($writeReq or $mirrorReq ) {
    ($prepareEnvelope, $seList) = $self->getSEsForWriteOrMirrorAccess($user,$prepareEnvelope,$seList,$sitename,$writeQos,$writeQosCount,$excludedAndfailedSEs, $prioritizedSEs, $deprioritizedSEs);
  }
