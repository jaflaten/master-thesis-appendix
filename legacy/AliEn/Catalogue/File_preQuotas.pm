...
#  my $tempname = $self->f_dirname($file);
  ...
  $self->{DATABASE}->{LFN_DB}->{FIRST_DB}->checkInsertOCDBTable($file) and
...
sub f_whereisWriteCache {
  my $self  = shift;
  my $cache = shift;
  scalar(@_) and $cache .= "&value=" . Dumper([@_]);
  
  if ($self->{CONFIG}->{CACHE_SERVICE_ADDRESS} and $cache) {
    my ($ok, $value) = AliEn::Util::getURLandEvaluate("$cache");
  }
...
sub createFileUrl {
  ...
  my ($newpath,$newguid) = $self->createFileName($se,$guid)
      or return;
  return ("$selectedprotocol/$newpath",$newguid,$se); #turl,guid,se
}
...
##                     remaining write accesses will be dropped, as they will fail anyway.
  ...
  # allowing to check for zero-sized files in order to get the current quota usage.
  # introduced to stop job submission at a certain breakpoint
...
"SELECT nbFiles, totalSize, maxNbFiles, maxTotalSize, tmpIncreasedNbFiles, tmpIncreasedTotalSize FROM FQUOTAS WHERE "
  ...
  $DEBUG and 
  $self->debug(1, "size: $size nbFile: $nbFiles/$tmpIncreasedNbFiles/$maxNbFiles 
...
sub fquota_list {
  ...
  my $result =
    ...
    and return -1;
  $result->[0] or $self->info("User $user does not exist in the FQUOTAS table",1)
    and return -1;
