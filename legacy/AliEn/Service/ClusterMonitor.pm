...
sub SetProcInfo {
  my ( $this, $queueId, $token, $procinfo) = @_;
  ...
  $self->info( "Set Procinfo for $queueId: $procinfo !! ".($token ? "with new JA" : "with old JA") );

#  my $done =$self->{SOAP}->CallSOAP("Manager/Job", "SetProcInfo",  $queueId, $procinfo);
  return $self->{LOCALJOBDB}->insertMessage($queueId, $token, "proc", $procinfo,0);
#  ($done) or return (-1, $self->{LOGGER}->error_msg()); 
...
sub RefreshProcInfo {
  my ( $this, $queueId, $token ) = @_;

  $self->info( "RefreshProcinfo for $queueId" );
  return $self->{LOCALJOBDB}->insertMessage($queueId, $token, "saving", "saving", 0);
}
...
sub changeStatusCommand {
  ...
  if (length($token) < 30 ){
   $self->info("Job $queueId has not token (next parameter $token)");
   @_=($token, @_);
   $token = "";
  }
  ...
  my $done =$self->{SOAP}->CallSOAP("Manager/Job", "changeStatusCommand", $queueId, $token, $oldstatus, $status, $self->{CONFIG}->{CE_FULLNAME}, $error, $node,"$nodeport");

  $done and $self->info("JM answered: ".Dumper($done->result));
...
sub putJobLog {
  ...
  return $self->{LOCALJOBDB}->insertMessage($queueId, $token, $tag, $message, 0);
}
