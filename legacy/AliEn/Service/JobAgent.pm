...
#		  and $self->info("Sending $var - $ENV{$var} to ML") 
		  and $self->{MONITOR}->sendParameters("$self->{CONFIG}->{SITE}_".$self->{SERVICENAME}, 
		                                       "$self->{HOST}:$self->{PORT}", 
		                                       { $var => $ENV{$var} });
		# We put the batchid(s) in the trace
	    if($ENV{ALIEN_PROC_ID} and !$self->{$ENV{ALIEN_PROC_ID}}->{$var} and $ENV{$var}){
	      $self->{$ENV{ALIEN_PROC_ID}}->{$var} = 1;
	      $self->putJobLog("trace", "BatchId $var: ".$ENV{$var});
	    }
	}
}
...
sub changeStatus {
  ...
  foreach my $data (split (/\s+/, $self->{VOs})) {
    ...
    $self->info("Contacting $org for $id");

    $self->info("Putting the status of $id to @print");
    $done = $self->{SOAP}->CallSOAP("CLUSTERMONITOR_$org", "changeStatusCommand", $id, $token, @_ );
    if ($self->{MONITOR}) {
    ...
    $self->info("Status change result CM ($_[1]): ".Dumper($done));
        
    if (!$done){
      print STDERR "Error contacting the ClusterMonitor\nGoing to the Manager/Job ($_[1])";
      $self->info("Error contacting the ClusterMonitor. Going to the Manager/Job ($_[1])");
      
      # We try few times to change status
      my $counter = 0;
      while($counter<3){
        my @arguments=@_;
        $done = $self->{SOAP}->CallSOAP("Manager/Job", "changeStatusCommand", $id, $token, shift @arguments, shift @arguments, $self->{CONFIG}->{CE_FULLNAME}, @arguments );
        $done and $done = $done->result;
        $self->info("Status change result JM($counter): ".Dumper($done));
        $done and last;
        sleep(5);
        $counter++;
      }
      
      $done or ($self->putJobLog("trace", "The job cannot change status to $_[1]") and return);
    }
    
    $done==-2 and $self->info("The job cannot authenticate with the token $token!") 
      and $self->putJobLog("trace", "The job cannot authenticate with the current token!") and return;
    
    $done and $self->{STATUS}=$_[1];
  }
  $self->info("Status changed to @print and status $self->{STATUS}");
...
sub putJobLog {
  ...
  my $joblog = $self->{SOAP}->CallSOAP("CLUSTERMONITOR","putJobLog", $id, $ENV{ALIEN_JOB_TOKEN}, @_) or return;
  return 1;
}
...
sub putAgentLog {
  ...
  if ($self->{last_agent_counter}){
    ...
    $self->{SOAP}->CallSOAP("CLUSTERMONITOR","putJobLog", $id, $ENV{ALIEN_JOB_TOKEN}, "agent",
        sprintf("%03d Last message repeated %d time(s)", $self->{agentlog_counter}, $self->{last_agent_counter}),@_) 
  }
  my $joblog = $self->{SOAP}->CallSOAP("CLUSTERMONITOR","putJobLog", $id, $ENV{ALIEN_JOB_TOKEN}, "agent", 
...
#  $catalog and  $catalog->close();
  ...
  $self->info("Received: queueId $result->{queueid} - token $result->{token} - user $result->{user}");

  $ENV{ALIEN_PROC_ID}=$self->{QUEUEID} = $result->{queueid};
  $ENV{ALIEN_JOB_TOKEN}                = $result->{token};
  $ENV{JALIEN_TOKEN_CERT}              = $result->{tokencert};
  $ENV{JALIEN_TOKEN_KEY}               = $result->{tokenkey};
...
#--- memory requirement
  ...
  if($ok){
  	...
  	$self->{MEMORY} = 8*1024*1024; #8GB default
  }
  ...
  foreach my $var (@env_variables) {
    ...
    $envvars = $envvars . " $var=$value,";
    $ENV{$var}=$value;  
  }
  if($ok and scalar(@env_variables)){
  	$envvars =~ s/,$//;
  	$self->putJobLog("trace", "Defining the environment variables:$envvars");
  }
...
sub executeCommand {
  ...
  $self->changeStatus("%",  "STARTED", $batchid,$self->{HOST}, $self->{PROCESSPORT} ) or return;
...
#  $catalog->close();
  ...
  $self->changeStatus("STARTED",$self->{STATUS},0,$self->{HOST},$self->{PORT}) or return;
  ...
  # In case we are running Singularity
  if($ENV{CE_SINGULARITY}){
  	my $env = "";
  	
  	foreach my $key (sort(keys %ENV)) {
  		$key =~ /ALIEN_/i and $env .= " export $key=\"$ENV{$key}\"; ";
  	}
  	
  	if($ENV{CE_SINGULARITY} !~ m/-c\s|--contain\s/){ 
  		$ENV{CE_SINGULARITY} =~ s/exec/exec -c -W $self->{WORKDIR}/;
  	}
  	  	
  	$s = $ENV{CE_SINGULARITY} . "  bash -c '$env " . $s."'";
  }
...
#	print POUT  "Delivered signals to the process    [#  ] : $tags[17]\n";
  ...
  open STDERR, ">&SAVEOUT2";
  $error=$error/256;
  $self->changeStatus($self->{STATUS}, "SAVING",$error) or return;
...
sub _getInputFile {
  ...
  for (my $i=0;$i<2;$i++) {
    $catalog->execute("get",$lfnName,"$pfn", "-x", $options ) and return 1;
    $options="";
...
sub getFilesFromInputCollection{
  ...
  foreach my $file (@inputData) {
    ...
    if ($type =~ /^collection$/) {
    	...
    	my ($localFile) = $catalogue->execute("get", "-x", $file2);
...
sub createZipArchives{
  ...
  foreach my $name (keys(%$archiveTable)) {
       ...
       if($dir ne "./"){
         `mkdir -p $dir`;
         -d $dir or $self->putJobLog("error", "The archive folder $dir could not be created, archive $file")
           and return;
       }
...
sub putFiles {
  ...
  foreach my $data (split (/\s+/, $self->{VOs})){
    ...
    $self->mkdir_output($self->{PROCDIR}) or return 0;
    ...
    foreach my $fs_table (@filesAndArchives) {
      ...
      foreach my $fileOrArch (keys(%$fs_table)) {
        ...
        if(!$no_links) {
          ...
          foreach my $file( keys %{$fs_table->{$fileOrArch}->{entries}}) {  # if it is a file, there are just no entries
            ($fs_table->{$fileOrArch}->{entries}->{$file}->{size} gt 0)
              ...
              and  $self->putJobLog("trace", "WARNING: not add the file. This warning is the only one and simply for your information, the job will proceed without further intervention.")
              and next;
            my ($fn, $fd) = fileparse($file);
            if($fd ne "./"){
              $self->mkdir_output($self->{PROCDIR}."/".$fd) or return 0;
            }
            push @list, join("###", "$self->{PROCDIR}/$file", $fs_table->{$fileOrArch}->{entries}->{$file}->{size},
            $fs_table->{$fileOrArch}->{entries}->{$file}->{md5},($guids{$file} || 0), ($fd ne "./" ? $fd : ""));
          }
          (scalar(@list) gt 0) and $links= join(";;",@list);
        }
        
        my ($fname, $fdir) = fileparse($fs_table->{$fileOrArch}->{name});
        if($fdir ne "./"){
          $self->mkdir_output($self->{PROCDIR}."/".$fdir) or return 0;
        }
        ...
        # In case files take long to upload, we send a heartbeat for each one (each 5 min at least)
        my $newtime = time;
        if ($newtime - $lastbeat > 5*60){
        	$self->info("Sending a heartbeat!");
      	  $self->{SOAP}->CallSOAP("CLUSTERMONITOR","RefreshProcInfo", $self->{QUEUEID}, $ENV{ALIEN_JOB_TOKEN});
      	  $lastbeat = $newtime;
        }
...
sub mkdir_output {
    ...
    while($counter){
      my $ec = $self->{UI}->execute("mkdir","-p",$dir);
      $ec and last;
      $counter--;
      sleep(5);
    }
    ...
    if(!$counter){
    	$self->putJobLog("trace", "Could not create OutputDir: $dir");
    	return 0;
    }
...
sub firstExecution {
  ...
  if ( ($procinfo) && ($procinfo ne "") ) { 
    ...
    my $done = $self->{SOAP}->CallSOAP("CLUSTERMONITOR","SetProcInfo",
				       $self->{QUEUEID}, $ENV{ALIEN_JOB_TOKEN}, "$procinfo");
...
sub lastExecution {
  ...
  if(!$killJob){
	  ...
	  my $done = $self->{SOAP}->CallSOAP("CLUSTERMONITOR","SetProcInfo", $self->{QUEUEID}, $ENV{ALIEN_JOB_TOKEN}, "$procinfo");
	#	$self->{SOAP}->checkSOAPreturn($done);
	...
	============================================================\n";
	  ...
	  # no need to store if ERROR_E, already done before
	  if($status_prev !~ /ERROR_E/){
		  # store the files
		  $self->{UI} or $self->{UI} = $self->getCatalogue();
		  if (!$self->{UI}) {
		      $self->info("Error getting an instance of the catalog");
		      $self->putJobLog("error","Could not get an instance of the LCM ($self->{HOST})");
		      $self->registerLogs();
		      $self->putJobLog("trace","Registered the JobLogOnClusterMonitor.");
		  } else {
...
sub checkWakesUp {
  ...
  $self->{CVMFS_NIOERR} or $self->{CVMFS_NIOERR}=0;  
  
  # Check CVMFS health
  if (!$ENV{SKIP_CVMFS_HEALTH_CHECK} and 
      !$self->{JOBLOADED} and 
      $self->{CONFIG}->{CE_INSTALLMETHOD} and $self->{CONFIG}->{CE_INSTALLMETHOD}=~"CVMFS"){
    
    $self->info("Going to check CVMFS health");
      	
    my $message = "OK";
    eval {  	
  	  $self->{CVMFS_NIOERR}   = `attr -qg nioerr /cvmfs/alice.cern.ch`;
  	  $self->{CVMFS_REVISION} = `attr -qg revision /cvmfs/alice.cern.ch`;
    };
    if($@ or !$self->{CVMFS_REVISION} or ($self->{CVMFS_REVISION} and $self->{CVMFS_REVISION} !~ /\d+/) ){
      $self->{CVMFS_NIOERR} = -1;
      $message = "Unknown (NFS?: $self->{CVMFS_REVISION})";
    }
        
    $self->{CVMFS_NIOERR}<=0 and $self->{MONITOR} and 
  	  $self->{MONITOR}->sendParameters("CVMFS_CHECK", "$self->{HOST}", 
                                     { 'Status' => $self->{CVMFS_NIOERR}, 'Message' => $message, 'Revision' => $self->{CVMFS_REVISION} });
    
  	my $count = 3;
  	while($self->{CVMFS_NIOERR}>0){
  	  # We have a CVMFS I/O error
  	  my $cvmfsstat;
  	  my $cvmfsconfig;
  	  my $cvmfsprobe;
  	  my $msg;
  	  eval {
  	  	$cvmfsstat   = `cvmfs_config stat -v alice.cern.ch`;
  	    $cvmfsconfig = `cvmfs_config showconfig alice.cern.ch`;
  	    $cvmfsprobe  = `cvmfs_config probe alice.cern.ch`;
  	  };
  	  if($@){
  	  	$msg = "--revision--\n$self->{CVMFS_REVISION}\n--nioerr--\n$self->{CVMFS_NIOERR}\n--cvmfs config--\nUnable to run cvmfs_config (cvmfs_config stat -v alice.cern.ch and/or cvmfs_config showconfig alice.cern.ch and/or cvmfs_config probe alice.cern.ch)";
  	  } else {
  	  	$msg = "--revision--\n$self->{CVMFS_REVISION}\n--nioerr--\n$self->{CVMFS_NIOERR}\n--cvmfs stat--\n$cvmfsstat\n--cvmfs config--\n$cvmfsconfig\n--cvmfs probe--\n$cvmfsprobe";
  	  }
  	  
  	  if($self->{MONITOR}){
  	  	$self->info("Sending to MonaLisa with CVMFS dumps: $msg");
  	  	  	  	
        $self->{MONITOR}->sendParameters("CVMFS_CHECK", "$self->{HOST}", 
                                           { 'Status' => $self->{CVMFS_NIOERR}, 'Message' => $msg, 'Revision' => $self->{CVMFS_REVISION} });  	  	
  	  }
  	  $count--;
  	  $count>0 or last;
   	  $self->info("Sleeping 120 seconds, $count tries left"); 	  
  	  sleep(120);  
  	  eval {  	
  	    $self->{CVMFS_NIOERR} = `attr -qg nioerr /cvmfs/alice.cern.ch`;
  	    $self->{CVMFS_REVISION} = `attr -qg revision /cvmfs/alice.cern.ch`;
      ...
      if($@){
        $self->{CVMFS_NIOERR} = -1;
        $self->{CVMFS_REVISION} = "Unknown (check: attr -qg revision /cvmfs/alice.cern.ch and attr -qg nioerr /cvmfs/alice.cern.ch)";
      }
  	}
  }
  ...
  if (! $self->{JOBLOADED}) {
    ...
    if ( (defined $self->{JOBLIMIT} and $self->{TOTALJOBS}>=$self->{JOBLIMIT}) || 
      ...
      $self->info("There are no jobs to execute (or we reached limit, or CVMFS error: $self->{CVMFS_NIOERR}). We have executed $self->{TOTALJOBS}.");
...
#      $self->{PROCINFO} = "$all[0] $all[1] $self->{AVCPU} $all[3] $cputime $all[5] $all[6] $all[7] $all[8] $all[9] $self->{MAXRESOURCECOST} $self->{AVRSIZE} $self->{AVVSIZE}";     	
  ...
  if ($self->{ALIEN_PROC_INFO} eq "10") {
    $self->{SOAP}->CallSOAP("CLUSTERMONITOR","SetProcInfo",
			    $self->{QUEUEID}, $ENV{ALIEN_JOB_TOKEN}, $self->{PROCINFO});
