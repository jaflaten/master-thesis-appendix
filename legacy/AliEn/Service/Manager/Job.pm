  ...
  if (!(defined $domainId) || !(@$domainId)) {
    ...
    $self->{DB}->insertSite(
      { siteName     => $domainSt->{OU},
        ...
        domain       => $domain,
        longitude    => $domainSt->{LONGITUDE} || 0,
        latitude     => $domainSt->{LATITUDE} || 0,
        record       => $domainSt->{RECORD} || "",
        url          => $domainSt->{URL} || ""
      }
      )
      or $self->{LOGGER}
      ->error("JobManager", "In InsertHost error inserting the domain $domainSt->{DOMAIN} in the database ($domain) with: ".Dumper($domainSt))
      and return;
...
sub enterCommand: Public {
  ...
  # check WorkDirectorySize
  $self->checkWorkDirectorySize($job_ca) != 0 or 
    return [-1, "The WorkDirectorySize is too big, the limit is 12000MB'"];
  ...
  # CPU cores
  ($ok, my $cpuCores) = $job_ca->evaluateAttributeString("CPUCores");
  $ok and $cpuCores and $set->{cpucores}=$cpuCores;
  ...
  # Packages
  my $pack_err = $self->checkenvPackages($job_ca, $procid);
  #$pack_err != 0 and return [-1, "Incompatible packages in the JDL. Try 'alienv checkenv pack1,pack2...'"];
  
  return $procid;
#    return [$procid, "Job inserted OK"];
}

# check the dependencies chain for the packages in CVMFS
sub checkenvPackages{
	my $this = shift;
	my $job_ca = shift;
	my $jobid = shift || "";
	my %checkenv;
	...
	($ENV{ALIEN_CHECKENV} and $ENV{ALIEN_CHECKENV}==1) or return 0;
	
	my ($ok,  @packages)=$job_ca->evaluateAttributeVectorString("Packages");
	$ok or return 0;
	my $packs = join(",", @packages);
	
	# we go through cache since this command is slow, in principle just global since there can be many ephemeral threads...	
	if(!defined $checkenv{code}){  
      if($self->{CONFIG}->{CACHE_SERVICE_ADDRESS}){
    	  $keyCache = "$self->{CONFIG}->{CACHE_SERVICE_ADDRESS}?ns=checkenv&key=".uri_escape("code_$packs");
		  ($ok, %checkenv) = AliEn::Util::getURLandEvaluate($keyCache, 1);
      }
      		
	  if(!$ok){
	  	($checkenv{output}, $checkenv{code}) = AliEn::Util::executeCommand("alienv checkenv $packs");	  		  	  		
	  		  	  		  
	    defined $checkenv{code} or $checkenv{code} = 0;
        $self->{CONFIG}->{CACHE_SERVICE_ADDRESS} and 
          AliEn::Util::getURLandEvaluate("$keyCache&timeout=5000&value=".Dumper([%checkenv]));
	  }
    }	
    
    if($checkenv{code} != 0){
      my $user = $job_ca->evaluateAttributeString("User");
      my $error = "";
      
      foreach (grep(/ERROR/, split(/\n/, $checkenv{output}))) {
        $error = $error.$_."\n";
      }
      
      my $userInfo = AliEn::Util::getUserInfoFromLDAP($user, $self->{CONFIG});
      my $userCN = $userInfo->get_value('cn');
      
      AliEn::Util::sendMail({
				to => $userInfo->get_value('email'), 
				cc => 'alice-grid-admins@cern.ch',
				from => 'alienmaster@cern.ch',
				subject => 'AliEn - Job submission - Package incompatibility detected',
				content => "Dear $userCN ($user),

    Your job $jobid requests incompatible packages in the JDL.

    $error
    You can check your packages by running the command '/cvmfs/alice.cern.ch/bin/alienv checkenv $packs' from lxplus or a machine with CVMFS installed.
    To see the list of dependencies, please go to http://alimonitor.cern.ch/packages/

    The Watchdog."
      });
    }
	
	return $checkenv{code};
}

sub checkWorkDirectorySize {
  my $this = shift;
  my $job_ca = shift;
  ...
  if ($okwork && defined $workspace[0] && $workspace[0] > 0) {
    ...
    if ($space > 12288000){ #12000MB
      ...
      AliEn::Util::sendMail({
				...
				cc => 'alice-grid-admins@cern.ch',
				from => 'alienmaster@cern.ch',
				subject => 'AliEn - Job submission - WorkDirectorySize limit exceeded',
				content => "Dear $userCN ($user),

    Your job submission requests a WorkDirectorySize of $space KB, and the limit is 12288000 KB (12000MB).
    Please modify your JDL and task so the needed scratch space fits in this limit.
...
sub getJobAgentRequirements {
  ...
  foreach my $entry ("user", "memory", "swap", "localdisk", "packages", "cpucores") {
...
sub SetProcInfo {
  ...
  if ($procinfo) {
    ...
    scalar(@values) > 1 or ($self->info("$procinfo is not an array for $queueId!") and return);
    ...
    my $updateRef = {
      runtime      => $values[0],
      runtimes     => int($values[1]),
      cpu          => $values[2],
      mem          => $values[3],
      cputime      => int($values[4]),
      rsize        => int($values[5]),
      vsize        => int($values[6]),
      ncpu         => int($values[7]),
      cpufamily    => int($values[8]),
      cpuspeed     => int($values[9]),
      cost         => $values[10],
      maxrsize     => int($values[11]),
      maxvsize     => int($values[12]),
      procinfotime => "$now",
      si2k         => $values[13]
    };
