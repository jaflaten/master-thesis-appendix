...
sub initialize {
  ...
  my ($host, $driver, $db) =
    split ("/", $self->{CONFIG}->{"CATALOGUE_DATABASE"});
    
  my $role="admin";
  $ENV{ALIEN_DATABASE_SSL} and $role.="ssl";
  if ( (defined $ENV{ALIEN_NO_PROXY}) && ($ENV{ALIEN_NO_PROXY} eq "1") && (defined $ENV{ALIEN_DB_PASSWD}) ) {
      $self->{CDB} = "AliEn::Database::Catalogue"->new({DB=>$db,HOST=> $host,DRIVER => $driver,ROLE=>$role,"USE_PROXY" => 0, PASSWD=>"$ENV{ALIEN_DB_PASSWD}"});
  } else {
      $self->{CDB} = "AliEn::Database::Catalogue"->new({DB=>$db,HOST=> $host,DRIVER => $driver,ROLE=>$role});
  }

  $self->{CDB}
    or $self->{LOGGER}->error( "Manager", "In initialize creating AliEn::Database::Catalogue instance failed" )
      and return;
...
sub enterTransfer {
  ...
  $self->debug(1, "Arguments:".Dumper($arguments));
  
  # finding the right destination if need to exclude a site
  if ($arguments->{excludeSite}) {
  	$self->info("Transfer needs to exclude site: ".$arguments->{excludeSite});
  	
    my $query = "SELECT DISTINCT s.seName, ifnull(distance, 0) + sedemotewrite weight FROM SE s left join SEDistance sed  
  				on (s.seNumber=sed.seNumber and sitename=?) 
  				where s.seName not like ? and s.seQoS like '%disk%' order by weight asc limit 1";
  				
  	my @queryValues= ($arguments->{excludeSite}, '%'.$arguments->{excludeSite}.'%');
  	
  	my ($result) = $self->{CDB}->{LFN_DB}->{FIRST_DB}->queryRow($query, undef, {bind_values => \@queryValues});
  	
  	$result or $self->info("enterTransfer: can't chose destination SE excluding $arguments->{excludeSite}") 
  	  and return(-1, "Mirror: can't chose destination SE excluding $arguments->{excludeSite}");
  	
  	$destination = $result->{seName};
  	$self->info("SE chosen: $destination");
  }
