...
sub SubmitSplitJob {
  ...
  if($ENV{JOBOPTIMIZER_DUMP_BASKETS}==1){
    $self->info("Baskets for queueid ".$queueid.": ".Dumper($jobs));
  }
...
#    $self->putJobLog($queueid, "trace", "Processing position $pos in loop $i ($counter)");
    ...
    if ($input) {
      ...
      if(!$self->{CATALOGUE}->{QUEUE}->checkRequirements($job_ca)){ 
        $self->putJobLog($queueid,"error","Subjob failed to submit, wrong requirements (more than 1000 InputData files?)");
        
        my ($okid, @patterns)=$job_ca->evaluateAttributeVectorString("InputData");
        $okid and $self->putJobLog($queueid,"error","InputData files: ".scalar(@patterns)) or
          $self->putJobLog($queueid,"error","Can't get InputData for subjob");
        
        my $lr = length $new_req;
        if($lr > 200){
          $self->putJobLog($queueid,"error","Requirements: ");
          my $total = 0;
          my $count = 1;
          while ($total < $lr){
          	my $mv = $count*200;
          	$lr < $mv and $mv = $lr;
          	$self->putJobLog($queueid,"error", substr($new_req, $total, $mv));
          	$count++;
          	$total = $total + 200;
          }
        } else {
          $self->putJobLog($queueid,"error","Requirements: ".$new_req);
        }
          
        $self->info("Requirements for subjob failed"); 
        next;
      }
