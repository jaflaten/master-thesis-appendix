...
#	open FILE, ">>", $self->{DIRFILES}."ocdb_${ocdbType}_".$time;
	...
	foreach my $lfn (@$lfnIds){
      ...
      if(!$localfile or (stat($localfile))[7] <= 0) {
...
#    close FILE;
    ...
    if($error==2){ 
    	$self->{SLEEP_PERIOD}=120; # we wait 2 minutes to retry in case of failed publication
		AliEn::Util::sendMail({
			to => "miguel.martinez.pedreira\@cern.ch", 
			from => 'alienmaster\@cern.ch',
			subject => 'AliEn - OCDB CVMFS publication failed',
			content => "Last publication command failed: $command - $failed ($dir)"
		});	
    }
    return 1;
}
