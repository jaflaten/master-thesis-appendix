...
sub copyInputCollection {
  ...
  foreach my $file (@inputData) {
    ...
    if(!$type){
    	$self->putJobLog($jobId, "error", "We couldn't get the type of collection? Please resubmit the masterjob.");
    	return;
    }
...
#    }   
    ...
    if( ($split and $#{$inputBox} > 50000) or (!$split and $#{$inputBox} > 10000) ){
    	$tag = "error";
    }
