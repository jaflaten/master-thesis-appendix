...
#    $self->{CONFIG}->{PACKMAN}->List;
	...
	$version or $version=
	  $self->{CONFIG}->{PACKMAN}->GetLatestVersion($pack);
...
sub InitializeSOAP {
  ...
  $self->{SOAP}->{CLUSTERMONITOR}=SOAP::Lite
    ->uri("AliEn/Service/ClusterMonitor")
      ->proxy("http://$ENV{ALIEN_CM_AS_LDAP_PROXY}");
  return $self->{SOAP};
}
...
sub InitializeLocalMSS {
    ...
    $name
      or $self->{LOGGER}->warning( "Command",
        "Warning! Data is not kept in the local mass storage system" )
      and return 1;
    ...
    $self->{MSS}
      or $self->{LOGGER}
      ->warning( "Command", "Error: getting an instance of $name" )
      and return;
...
sub changeStatus {
	...
	foreach my $data (split (/\s+/, $ENV{ALIEN_VOs})) {
		...
		my $done = $self->{SOAP}->CallSOAP("CLUSTERMONITOR_$org", "changeStatusCommand", $id, $token, @_);
		
		$done and $done=$done ->result;
		if (!$done){
			
			print STDERR "Error contacting the ClusterMonitor\nGoing to the Manager/Job";
			my $done =    $self->{SOAP}->CallSOAP("Manager_Job_$org", "changeStatusCommand",  $id, $token, @_);
		}
	}
}
...
sub Validate {
    ...
    $self->{CATALOGUE}=AliEn::UI::Catalogue::LCM->new({"silent", "0",
						       "debug", 0});
...
sub ParseOutput{
  ...
  foreach $file (@files) {
	...
	or print STDERR "Error opening the file $dir/stdout!!" and 
	    $self->ValidateError();
