...
sub _system {
  ...
  eval {
    alarm(300);
    my $pid = open(FILE, "$command |")
      or die("Error doing '$command'!!\n$!");
    @output = <FILE>;
...
sub isValidGUID {
  ...
  (length($guid) eq 36)
    and $lines = substr($lines, 8, 1) . substr($lines, 13, 1) . substr($lines, 18, 1) . substr($lines, 23, 1)
    and $lines =~ s/[-]*//
    and (length($lines) eq 0)
    and $guid =
        substr($guid, 0, 8) . substr($guid, 9, 4) . substr($guid, 14, 4) . substr($guid, 19, 4) . substr($guid, 24, 12)
    and $guid =~ s/[0-9a-f]*//i
...
=begin usage
 my $mail = (
          ...
          cc => "first@cc.com[, second@cc.com]", # optional
          bcc => "hidden@bcc.com",               # optional    
 )
=cut
...
sub sendMail {
  ...
  ($options->{to} and $options->{from} and $options->{content} and $options->{subject})
    or return -1;
 
 # create the lwp user agent
 use LWP::UserAgent;
 ...
 # Create a request                                                                                                                                                                                                                                                                              
 my $req = HTTP::Request->new(POST => "mailto:$options->{to}");
 $req->header(Subject => $options->{subject});
 $options->{cc}  and $req->header(cc => $options->{cc});
 $options->{bcc} and $req->header(bcc => $options->{bcc}); 
 
 $req->content($options->{content});

 my $res = $ua->request($req) or return -1;
 return $res;
}


# returns the blessed Net::LDAP::Entry - first element out of the collection that matches the uid (should be unique anyways)
# or zero, if no matching entries were found.

# Access returned entries:
# my $entry = AliEn::Util::getUserInfoFromLDAP($user);
# $entry->get_value('someField');
sub getUserInfoFromLDAP {
  
  my $user = shift;
  my $config = shift || AliEn::Config->new();
  # get the user's email from LDAP
  my $ldap = Net::LDAP->new($config->{LDAPHOST}) or return 0;
	$ldap->bind;     
  
  my $mesg = $ldap->search( base   => "ou=People,$config->{LDAPDN}",
          filter => "(&(objectclass=AliEnUser) (uid=$user))",
          attrs => ['*']);
  
  ($mesg->count) or return 0;
  return ($mesg->entries)[0];

}

# execute system call retrieving both output and exit code
sub executeCommand {
  my $command = join ' ', @_;
  ($_ = qx{$command 2>&1}, $? >> 8);
}
