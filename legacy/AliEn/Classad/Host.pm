...
sub setPackages {
  ...
  if ( $self->{CONFIG}->{CE_INSTALLMETHOD} and $self->{CONFIG}->{CE_INSTALLMETHOD}=~"CVMFS" ) {
      ...
      my ($status, @packages)=$self->{PACKMAN}->getListPackages("-s");
      if (@packages) {
        $self->debug(1, "List of packages to @packages");
        ...
        $self->info("Error getting CVMFS packages!! Please check the output of running: 'alienv q' !");
      }
  } else {  
