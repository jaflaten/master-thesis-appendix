...
sub getSubject {
    ...
    if ( !( $self->{IS_LOADED} ) ) {
        print
          "The certificate is not loaded. call X509->load(filename) first\n";
        return;
    }
...
sub getRemainingProxyTime {
  ...
  open (FILE,"$ENV{GLOBUS_LOCATION}/bin/grid-proxy-info 2>&1|") or
    $self->debug(1, "Error doing $ENV{GLOBUS_LOCATION}/bin/proxy-info") and return 0;
  my @data=<FILE>;
  close FILE;
