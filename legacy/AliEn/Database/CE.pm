...
sub insertMessage {
  ...
  $message=~ s/\n/\\n/g;
  print FILE "$jobId\t$jobToken\t$tag\t$time\t$message\n";
  close FILE;
...
sub retrieveMessages{
  ...
  foreach my $file (readdir(IMD)) {
    ...
    foreach my $l (<FILE>){
      $lines++;
      my ($id, $token, $tag, $time, $message)=split(/\t/, $l, 5);
      (defined $message and length $message) or $self->info("Skipping message: $id $token $tag $time") and next;
      push @$result, {jobId=>$id, jobToken => $token, tag=>$tag ,time=>$time, procinfo=>$message};
    }
    close FILE;
    unlink "$dir/$file";
