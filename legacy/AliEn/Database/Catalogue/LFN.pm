...
sub createCatalogueTables {
  ...
  my %tables=(HOSTS=>["hostIndex", {hostIndex=>"serial primary key",
		...
		SEDistance => ["sitename",
			     { sitename => "varchar(32) collate latin1_general_ci  not null",
        ...
        FQUOTAS=>["user",{user=>"varchar(64) NOT NULL",
            ...
            maxNbFiles=>"bigint(20) NOT NULL DEFAULT '0'",
            nbFiles=>"bigint(20) NOT NULL DEFAULT '0'",
            tmpIncreasedTotalSize=>"bigint(20) NOT NULL DEFAULT '0'",
            maxTotalSize=>"bigint(20) NOT NULL DEFAULT '0'",
            tmpIncreasedNbFiles=>"bigint(20) NOT NULL DEFAULT '0'"},
...
$options->{retrieve} =~ s{guid}{$b as guid};
  ...
  my $retrieve=($options->{retrieve} or "entryId,owner,replicated,guidtime,aclId,  broken, expiretime, ".$self->reservedWord("size").",dir,  gowner,  ".$self->reservedWord("type")." ,md5,perm,concat('$tablePath',lfn) as lfn, ".$self->binary2string." as guid,".$self->dateFormat("ctime").", jobid");
...
sub createFile {
  ...
  my $seNumbers={};
  my @inserts=@_;
  my $totalInsertsSize=0; # add the size of all inserts together in order to update the file quota
  
  foreach my $insert (@inserts) {
    $insert->{type} or $insert->{type}='f';
    $entryDir or $entryDir=$self->getParentDir($insert->{lfn});
    $insert->{dir}=$entryDir;
    $totalInsertsSize += $insert->{size};
 ...
 #Update quota
 $self->fquota_update($totalInsertsSize,scalar(@inserts), $inserts[0]->{owner});
 return $done;
}
...
sub removeFile {
  ...
  # look up the current owner of the file. The quota of it's owner should be reduced and not the quota of the user removing this file
  my $owner = $db->queryValue("SELECT owner FROM $tableName WHERE lfn=?", undef, {bind_values=>[$lfnOnTable]});  
  ...
  if($filehash->{type} eq "f"){
    $self->fquota_update(-1*$filehash->{size},-1, $owner)
...
sub removeDirectory {
  ...
  foreach my $db (@$entries) {
    ...
    $owner or $owner = $db2->queryValue("SELECT owner FROM L$db->{tableName}L WHERE lfn LIKE concat(?,'%') limit 1",
  ...
  $self->fquota_update(-$size,-$count,$owner) 
    or $self->{LOGGER}->error("Database::Catalogue::LFN","ERROR: Could not update quotas")
    and return;
  return 1;
}
...
sub getDiskUsage {
  ...
  if ($lfn=~ m{/$}){
    ...
    foreach my $entry (@$hosts){
      ...
      defined $partialSize or $partialSize=0;
...
#  $self->do("alter table $table drop primary key");
  ...
  foreach my $entry( @newlist){
    ...
    my $done=$self->do("update $table set dir=dir-? where dir>=? $max1 order by entryId", {bind_values=>$bind});
  ...
  if ($options->{min} and $options->{min}>1){
    ...
    $self->do("update $table set entryId=entryId+$options->{min}-1, dir=dir+$options->{min}-1 order by entryId desc");
    $changes=1;
  }
...
sub getAllHostAndTable{
  ...
  my $result = $self->query("SELECT distinct hostIndex, tableName, lfn from INDEXTABLE");
  defined $result
...
sub fquota_update {
  ...
  (defined $size) and (defined $count) and (length($user)>0) or $self->info("Update fquota : not enough parameters") and return;

  if($user and $user !~ /aliprod/ and $user !~ /alitrain/ and $user !~ /alidaq/){
    $self->info("Updating file quotas of user $user");
    $self->do("UPDATE $self->{CONFIG}->{CATALOG_DATABASE}.FQUOTAS SET nbFiles=nbFiles+tmpIncreasedNbFiles+?, totalSize=totalSize+tmpIncreasedTotalSize+?, tmpIncreasedNbFiles=0, tmpIncreasedTotalSize=0 WHERE user=?", {bind_values=>[$count,$size,$user]}) or return;
  }
  return 1;
}
