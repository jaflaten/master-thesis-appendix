...
sub createCatalogueTables {
  ...
  my %tables=(HOSTS=>["hostIndex", {hostIndex=>"int(11) NOT NULL auto_increment primary key",
				    address=>"char(50)", 
				    db=>"char(40)",
				    driver=>"char(10)", 
...
sub moveGUIDs {
  ...
  if ($self->{HOST} eq $db->{HOST} and $self->{DRIVER} eq $db->{DRIVER}){
    ...
    my @queries=("INSERT INTO $self->{DB}.G${tableName}L ($columns) select $columns from $table where  binary2date(guid)>string2date('$guid')",
		...
		"INSERT INTO $self->{DB}.G${tableName}L_REF (guidId, lfnRef) select p.guidId, p.lfnRef from ${table}_REF p, $self->{DB}.G${tableName}L g where p.guidId=g.guidId",
		"delete from p using ${table}_REF p left join $table g on p.guidId=g.guidId where g.guidId is null");
...
#  }
  if ($error){
    $self->info("WE SHOULD REMOVE THE INDEX $tableName \n");
    return;
  }
...
sub updateStatistics {
  ...
  $self->do("insert into GL_STATS(tableNumber, seNumber, seNumFiles, seUsedSpace) select ?, seNumber, count(*), sum(".$self->reservedWord("size").") from $table g ,${table}_PFN p where g.guidid=p.guidid group by senumber", {bind_values=>[$index]});
  return 1;
}
