...
sub CallAndGetOverSOAP{
  ...
  my $maxTry = 3;
  my $tries;
  for ($tries = 0; $tries < $maxTry; $tries++) { # try three times 
    $callsoap=$self->CallSOAP(@signature) and last;
    $self->info("Calling $service over SOAP did not work, trying ".(2 - $tries)." more times till giving up.");
    sleep(($maxTry*$tries));
  }
  if ( $tries == $maxTry ){
